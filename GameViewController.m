...
- (void)calculateLabelFrames
{
    ...
    if (player != nil) {
        ...
        CGPoint winsPoints = point;
        winsPoints.x += labelWidth/2.0f + 6.0f + 19.0f;
        winsPoints.y -= 0.5f;
        self.playerWinsBottomLabel.center = winsPoints;
    ...
    if (player != nil) {
        ...
        winsPoints.x += labelWidth/2.0f + 6.0f + 19.0f;
        winsPoints.y -= 0.5f;
        self.playerWinsLeftLabel.center = winsPoints;
    ...
    if (player != nil) {
        ...
        winsPoints.x += labelWidth/2.0f + 6.0f + 19.0f;
        winsPoints.y -= 0.5f;
        self.playerWinsTopLabel.center = winsPoints;
    ...
    if (player != nil) {
        ...
        winsPoints.x += labelWidth/2.0f + 6.0f + 19.0f;
        winsPoints.y -= 0.5f;
        self.playerWinsRightLabel.center = winsPoints;
...
{
    NSTimeInterval ping = [game ping];
    NSLog(@"The Ping was: %f", ping);
    ping = 0.35;
    NSLog(@"The Ping is: %f", ping);
    [_turnCardSound play];
    CardView *cardView = [self cardViewForCard:card];
    if (game.isServer) {
        self.snapButton.enabled = (player.position != PlayerPositionBottom);
        if (player.position == PlayerPositionBottom) {
            [cardView performSelector:@selector(animateTurningOverForPlayer:) withObject:player afterDelay:ping*1.3];
            NSLog(@"Flip Server Bot");
        } else {
            [cardView performSelector:@selector(animateTurningOverForPlayer:) withObject:player afterDelay:ping*1.3];
            NSLog(@"Flip Server non");
        }
    } else {
        if (player.position == PlayerPositionBottom) {
            [cardView performSelector:@selector(animateTurningOverForPlayer:) withObject:player afterDelay: ping*2];
            NSLog(@"Flip Client bot");
        } else {
            [cardView performSelector:@selector(animateTurningOverForPlayer:) withObject:player afterDelay: 0];
            NSLog(@"Flip client non");
        }
    }
}
...
{
    self.centerLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%@ Must Pay", @"Status text: player must pay card to the others"), player.name];
